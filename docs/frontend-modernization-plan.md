...
- **Rollout:** Incremental, route-by-route. Old HAML and new React coexist behind the same Rails app until every screen is ported.
- **Keep the entire existing site in place until switchover.** The current HAML site stays **fully functional and live** — nothing is removed, retired, or degraded — while the React site is built alongside it. New React pages are added without deleting their HAML equivalents, and we only cut a route (or the whole site) over to React once its replacement is complete and verified. No legacy code (HAML views, Webpacker packs, Bootstrap 4, Sprockets) is deleted until after a successful cutover. See **Cutover strategy** below.
...
- **SPA lives in `app/frontend/`** (Vite Ruby default), cleanly separate from legacy `app/javascript/` (Webpacker) and `app/assets/` (Sprockets) so all three coexist during migration.
- Add `gem "vite_rails"`; run `bundle exec vite install` (generates `config/vite.json`, `bin/vite`, `Procfile.dev`). Keep the `webpacker` gem until its two packs are ported (M6), then remove it.
...
- **Postcode lookup — keep postcodes.io, do not couple to TV.** swapmyvote already resolves postcodes via the free, maintained public **postcodes.io** API (`app/javascript/packs/postcodesHelper.js` reads `codes.parliamentary_constituency_2024` — the exact ONS code it keys on) and has no backend lookup service. TV's `/api/lookup` is not a shareable service: no CORS, unversioned (returns TV-specific `slug` first), a hidden `DC_API_KEY` (Democracy Club) dependency for multi-constituency/address cases, and Next.js-serverless-bound (Vercel `maxDuration`/512 MB). Consuming it would add a fragile runtime dependency on another team's app — a downgrade. In M3 we simply reimplement the ~90-line jQuery helper as a React `PostcodeLookup` component **still calling postcodes.io**.
...
- **M4 — Constituency / profile edit.** Port `user/constituencies` + `users#edit/update`. API: `ConstituenciesController`, `UsersController#update`.
...
- **M6 — Mobile verification.** Port `MobilePhoneController` → API; build a React phone input + OTP form to replace the `intlTelInput.js` pack. The React version supersedes the pack, but **leave the Webpacker pack + `app/javascript/` in place** (the legacy HAML mobile page keeps using it) — removal happens only at cleanup, after cutover.
...
- **M9 — Cutover.** Once every user-facing screen has a verified React replacement, flip the remaining routes to the SPA (see **Cutover strategy**). Only *after* a successful cutover: remove Bootstrap 4 CDN + Sprockets stylesheets, the Webpacker gem + `app/javascript/`, CoffeeScript/`Gruntfile.coffee`, and retire `application.html.haml`. Admin (`admin#index/stats`) may stay HAML indefinitely (internal, low value) — if so, keep the minimal legacy layout it needs.
...
- **Gate the switch, per route, reversibly.** Whether a path serves React or HAML is controlled by the allow-list (optionally driven by an env flag such as `SWAPMYVOTE_REACT_ROUTES`), so we can enable React for a route in staging, verify, and flip production — or revert instantly to HAML if something regresses — without a code change/deploy where feasible. This lets us cut over gradually (route by route) or all at once, our choice, when ready.
- **Verify before flipping each route.** A route only switches to React once its React version passes the milestone's Vitest + RSpec request specs, the Playwright/axe checks, and manual end-to-end verification against the real flow. Until then the route keeps serving HAML in production.
- **Defer all deletion to M9 cleanup, post-cutover.** No removal of HAML views, Webpacker packs, `app/javascript/`, Bootstrap 4/Sprockets, or CoffeeScript until the new site is fully switched over and stable. (Note: the legacy Webpacker build breaks on Node ≥17 — see risk #8 — so during coexistence the legacy packs are kept building via `NODE_OPTIONS=--openssl-legacy-provider`; they are only removed at cleanup.)
...
2. **OmniAuth redirect flow.** It's a leave-the-SPA full-page POST, not XHR; land the callback on a SPA route that re-hydrates via session. Test Twitter+Facebook round-trip early in M5 with `Identity.from_omniauth` untouched.
...
6. **SEO of static pages.** Client-rendered SPA loses server HTML for faq/about/terms. **Recommendation: keep production static routes on HAML** (already SSR, low-churn) or migrate them last; M1 is a toolchain proof, not necessarily a production cutover. The eventual standalone-SPA deploy restores SSR. **Confirm this trade-off with the team.**
7. **Decommissioning jQuery/Coffee widgets.** `postcodesHelper.js` (postcode→constituency, `parliamentary_constituency_2024` lookup) and `intlTelInput.js` (phone validity) embed real validation — port as React components with unit tests replicating exact checks (M3, M6) before deleting packs + webpacker gem.
