...
4. ~~**`rspec-rails` 4 → 8 is four majors, taken in two hops.**~~ **Overtaken by step 2.** The 4.0.1 → 6.1.5 hop (with `rspec-core` 3.9.3 → 3.13.6) needed **no spec changes at all** — 579 examples, 0 failures, no deprecations. The anticipated churn in `spec/rails_helper.rb` and the controller specs did not materialise. The second hop to 8.x in step 3 should be treated as low risk on that evidence, though it is still worth keeping out of the same commit as the Rails bump so a failure stays attributable.
