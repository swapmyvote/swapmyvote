...
4. **The review screen drops the hard-coded election name.** The legacy copy reads "Predicted GE2024 results for …"; the React version reads "Predicted results for …". Deliberate: `GE2024` does not generalise to a by-election, and the constituency name plus the surrounding page chrome already say which election this is.
5. **The React screens add an `h1` where the legacy screens have none.** `ConstituencyForm`, `ProfileForm` and the review screen's pages all render a heading the HAML versions never had. A WCAG improvement (every page needs exactly one `h1`), not drift to reconcile at cutover.
6. **`/app/constituency` carries no swapping-open gate**, even though `User::ConstituenciesController#edit` has `before_action :require_swapping_open`. Point 1 above already ungates `PATCH /api/v1/user` deliberately; `GET /api/v1/constituencies/:ons_id` is public reference data by the same reasoning (point 2's serializer split exists partly so `#index` stays untouched by it). The `/app/constituency` screen simply inherits that — nobody re-added a gate the API doesn't enforce. Worth a second look at M9: the legacy screen *is* gated, so cutover should either add the gate here or confirm the ungating was meant to reach the screen too.
