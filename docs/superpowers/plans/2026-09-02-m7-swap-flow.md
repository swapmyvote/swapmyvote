...
- **Reject with a real `ApiError` when testing a failed request.** `apiErrorMessages` branches on `error instanceof ApiError`, so a shaped look-alike (`Object.assign(new Error(), { name: "ApiError" })`) always falls through to the generic fallback and the test proves nothing. Construct `new ApiError(status, { error: { code, messages, fields } })`, as `LoginForm.test.tsx` does.
...
export function useSwap(enabled = true): UseQueryResult<SwapDetail | null> {
  return useQuery({
    ...
    // logged-in user, and refetchInterval would otherwise keep 401ing every 15
    // seconds behind RequireLogin's prompt, which never unmounts this page.
    enabled,
    staleTime: 5_000,
...
describe("SwapNew", () => {
  ...
  it("reports what the server refused", async () => {
    // A real ApiError, not a shaped object: apiErrorMessages branches on
    // `instanceof ApiError`, so a look-alike silently falls through to the
    // generic fallback and the test proves nothing.
...
describe("ShareEmailConsentForm", () => {
  ...
  it("surfaces the message the server refused with", async () => {
    // A real ApiError, not a plain Error: apiErrorMessages branches on
    // `instanceof ApiError`, so anything else only ever proves the generic
    // fallback renders.
    mutateAsync.mockRejectedValue(
      new ApiError(422, {
        error: {
