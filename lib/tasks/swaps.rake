namespace :swaps do
	...
	task :csv => :environment do
		...
		Swap.where(confirmed: true).each {|s|
			print "#{s.choosing_user.try(:name)},#{s.choosing_user.try(:email)},#{s.choosing_user.try(:constituency).try(:name)},#{s.choosing_user.try(:willing_party).try(:name)},#{s.id}\n"
			print "#{s.chosen_user.try(:name)},#{s.chosen_user.try(:email)},#{s.chosen_user.try(:constituency).try(:name)},#{s.choosing_user.try(:willing_party).try(:name)},#{s.id}\n"
		}
	end
